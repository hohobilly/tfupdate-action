...
function run_tfupdate {
  ...
  if git add . && git diff --cached --exit-code --quiet; then
    ...
    if [ ${INPUT_REVIEWER} ]; then
      hub pull-request -m "${UPDATE_MESSAGE}" -m "${PULL_REQUEST_BODY}" -b "${INPUT_BASE_BRANCH}" -l "${INPUT_LABEL}" -r "${INPUT_REVIEWER}" -p -h "${CHECKOUT_BRANCH}" -b "${${INPUT_BASE_BRANCH}}"
    else
      hub pull-request -m "${UPDATE_MESSAGE}" -m "${PULL_REQUEST_BODY}" -b "${INPUT_BASE_BRANCH}" -l "${INPUT_LABEL}" -p -h "${CHECKOUT_BRANCH}" -b "${${INPUT_BASE_BRANCH}}"
    fi
  fi
}
